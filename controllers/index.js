...
async function getPersonalData(req, res) {
  try {
    ...
    const data = await collection.find().toArray(); // Let's find everything in our collection and return it as an array
    person = {
      data: 'hi',
      professionalName: 'Dubois',
    };
    res.status(200).send(data); // Send the data as a response
  } catch (err) {
    console.error(err);
